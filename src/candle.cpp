    ...
    bus(bus)
{
    ...
    if (bus->getType() == mab::BusType_E::USB)
        vout << "CANdle at " << bus->getDeviceName() << ", ID: 0x" << std::hex << getDeviceId() << std::dec
             << " ready (USB)" << std::endl;
...
bool Candle::addMd80(uint16_t canId, bool printFailure)
{
    ...
    *(uint16_t*)payload = canId;
    ...
    if (sendBusFrame(BUS_FRAME_MD80_ADD, 2, payload, 3, 2))
    {
        ...
        {
            vout << "Unable to read MD80's firmware version! Please check the ID, or update the MD80 with "
                    "MAB_CAN_Flasher."
                 << statusFAIL << std::endl;
            return false;
        }
        ...
        if (firmwareVersion.i < md80CompatibleVersion.i)
        {
            ...
            firmwareVersion.s.major > md80CompatibleVersion.s.major ||
...
std::vector<uint16_t> Candle::ping(mab::CANdleBaudrate_E baudrate)
{
    ...
    if (sendBusFrame(BUS_FRAME_PING_START, 2000, nullptr, 2, 33))
    {
        ...
        for (size_t i = 0; i < ids.size(); i++)
        {
            ...
            vout << std::to_string(i + 1) << ": ID = " << ids[i] << " (0x" << std::hex << ids[i] << std::dec << ")"
                 << std::endl;
            if (ids[i] > idMax)
            {
...
bool Candle::sendGenericFDCanFrame(uint16_t canId, int msgLen, const char* txBuffer, char* rxBuffer, int timeoutMs)
{
    ...
    if (bus->transmit(tx, len, true, timeoutMs, 66, false)) // Got some response
    {
        if (*bus->getRxBuffer(0) == tx[0] &&                                   // USB Frame ID matches
            *bus->getRxBuffer(1) == true && bus->getBytesReceived() <= 64 + 2) // response can ID matches
        {
    ...
    bool             canTermination)
{
    ...
    vout << "It's CAN timeout (watchdog) is now " << (newTimeout == 0 ? "disabled" : std::to_string(newTimeout) + "ms")
         << std::endl;
...
bool Candle::configMd80Save(uint16_t canId)
{
    return executeCommand(
        canId, Md80Reg_E::runSaveCmd, "Saving in flash failed at ID: ", "Saving in flash successful at ID: ");
}
...
bool Candle::configCandleBaudrate(CANdleBaudrate_E canBaudrate, bool printVersionInfo)
{
    ...
    if (sendBusFrame(BUS_FRAME_CANDLE_CONFIG_BAUDRATE, 50, payload, 2, 6))
    {
        ...
        if (printVersionInfo)
        {
            ...
            {
                vout << "Your CANdle device firmware seems to be out-dated. Please see the manual for intructions on "
                        "how to update."
                     << std::endl;
                return false;
            }
...
bool Candle::configMd80TorqueBandwidth(uint16_t canId, uint16_t torqueBandwidth)
{
    if (inUpdateMode() ||
        !md80Register->write(
            canId, Md80Reg_E::motorTorgueBandwidth, torqueBandwidth, Md80Reg_E::runCalibratePiGains, true))
    {
...
{
    return executeCommand(
        canId, Md80Reg_E::runCalibrateCmd, "Starting calibration failed at ID: ", "Starting calibration at ID: ");
}
...
bool Candle::setupMd80DiagnosticExtended(uint16_t canId)
{
    ...
    if (inUpdateMode() || !md80Register->read(
                              canId,
                              Md80Reg_E::motorName,
                              regR.RW.motorName,
                              Md80Reg_E::buildDate,
    ...
    {
        vout << "Extended diagnostic failed at ID: " << canId << " while reading outputEncoderCalibrationMode register"
             << std::endl;
        return false;
    }
    if (!md80Register->read(canId, Md80Reg_E::motorCalibrationMode, regR.RW.motorCalibrationMode))
    {
        vout << "Extended diagnostic failed at ID: " << canId << " while reading motorCalibrationMode register"
             << std::endl;
        return false;
    }
    ...
    {
        vout << "Extended diagnostic failed at ID: " << canId << " while reading position limits registers"
             << std::endl;
        return false;
    }
    ...
    {
        vout << "Extended diagnostic failed at ID: " << canId << " while reading acceleration control data registers"
             << std::endl;
        return false;
    }
...
std::string getVersionString(const version_ut& ver)
{
    if (ver.s.tag == 'r' || ver.s.tag == 'R')
        return std::string(
            std::to_string(ver.s.major) + '.' + std::to_string(ver.s.minor) + '.' + std::to_string(ver.s.revision));
    else
        return std::string(
            std::to_string(ver.s.major) + '.' + std::to_string(ver.s.minor) + '.' + std::to_string(ver.s.revision) +
            '.' + ver.s.tag);
